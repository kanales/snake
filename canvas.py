...
<div>
<canvas id="{0}" width="{1}" height="{2}" style="background:rgba(0,0,0,1);" onclick='click_callback(this, event, "{3}")'></canvas>
</div>
...
class Canvas:
    ...
    def __init__(self, varname, width=800, height=600, cid=None):
        ...
        self.width = width
        self.height = height
        self.html = _canvas.format(
            self.cid, self.width, self.height, self.name)
        self.exec_list = []
    ...
    def arc(self, x, y, r, start, stop):
        ...
        self.execute("arc({0}, {1}, {2}, {3}, {4})".format(
            x, y, r, start, stop))
